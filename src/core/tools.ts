...
export async function computeCurrentNetworkSpeedHandler(
  ...
  const file = map.get(UPLOAD_FILE_CONST) as unknown as File,
    { size: fileSize, name: fileName } = file,
    computedSize = Math.min(COMPUTE_NETWORK_BYTE_SIZE, fileSize),
    fileExt = fileName.split(".").pop() || "tmp";

  // 构建参数 - 使用专用前缀避免与真实文件 hash 冲突
  const calculationHashCode = `${SPEED_TEST_PREFIX}${uniqueCode}`;
 ...
 *
 * @author lihh
 * @param uniqueCode 唯一的 code
 * @param key 键
 * @param value 值
 */
export function setGlobalInfoMappingHandler(
  uniqueCode: string,
  key: string,
  value: unknown,
) {
  let map;
  // 判断 map 是否存在
  if (!isHas(globalInfoMapping, uniqueCode))
    // 设置默认值
    map = globalInfoMapping[uniqueCode] = new Map();
  else map = globalInfoMapping[uniqueCode];

  map.set(key, value);

  // 针对 fileSize/ 以及消费偏移量 做处理
  if (SYNC_NUMERIC_VALUE_KEYS.has(key)) {
    const globalInfo = equals(CURRENT_CONSUME_BYTES, key)
      ? globalConsumeOffset.current
      : globalFileSize.current;
    globalInfo.set(uniqueCode, Number(value));
  }
}
 ...
 * @param uniqueCode 唯一的 code
 */
export function deleteGlobalInfoMappingHandler(uniqueCode: string) {
 ...
 *
 * @author lihh
 * @param args 传递的参数。如果只有一个参数的话，就是删除，多个参数则成对设置为添加
 */
export function putGlobalInfoMappingHandler(...args: Array<unknown>) {
  ...
  if (args.length == 1) {
    ...
    for (let i = 0; i < params.length; i += 2) {
      setGlobalInfoMappingHandler(code, params[i] as string, params[i + 1]);
    }
  }
}
...
export function generateBaseProgressState(
  ...
  const baseQueueElement: Required<QueueElementBase> = {
    ...
    fileName: map.get("fileName") as string,
    progress: 0,
    retryTimes: 0,
 ...
 */
(function () {
  // 判断 worker 是否已经存在或条件不满足，直接警告并返回
  if (
    isNotEmpty(calculateNameWorker.current) ||
    isUndefined(Worker) ||
    // @ts-ignore
