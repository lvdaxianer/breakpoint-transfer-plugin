...
channel.port2.onmessage = async function (event) {
  ...
  const newFileName = getNameHandler();
  // 如果有扩展名则添加，否则直接使用新文件名
  const message = suffix ? `${newFileName}.${suffix}` : newFileName;
...
export async function startUploadFileHandler(
  ...
  const calculationHashCode = calculationHashName.includes(".")
    ? calculationHashName.split(".").shift()!
    : calculationHashName;
...
export function uploadHandler(
  ...
  return new Promise<ProgressReturnType>(async (resolve, reject) => {
    ...
    putGlobalInfoMappingHandler(
      uniqueCode,
      UPLOAD_CONFIG_KEYS.FILE_NAME,
      fileName,
      UPLOAD_CONFIG_KEYS.UNIQUE_CODE,
      uniqueCode,
      UPLOAD_CONFIG_KEYS.FILE_SIZE,
      fileSize,
      UPLOAD_CONFIG_KEYS.EXT_NAME,
      extName,
      UPLOAD_CONFIG_KEYS.UPLOAD_FILE,
      uploadFile,
      // 此时表示内部进度
      UPLOAD_CONFIG_KEYS.INNER_PROGRESS,
      0,
      // 表示当前消费的字节数
      UPLOAD_CONFIG_KEYS.CURRENT_CONSUME_BYTES_KEY,
      0,
      UPLOAD_CONFIG_KEYS.NETWORK_SPEED,
...
uploadHandler.lng = async function (language?: LanguageEnumType) {
  // 设置 当前选择的语言 - 支持的语言枚举值
  const validLanguages = Object.values(LanguageEnumType);
  const chooseLanguage =
    isEmpty(language) || !validLanguages.includes(language!)
      ? LanguageEnumType.ZH
      : language!;
