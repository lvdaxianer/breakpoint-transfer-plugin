...
emitterAndTaker.on(
  ...
  async function (el: QueueElementBase) {
    ...
    // 这里监听 正常结束的状态，就是为了执行正常的回调方法
    switch (el.type) {
      case UploadProgressState.Done:
      case UploadProgressState.QuickUpload:
        const map = globalInfoMapping[el.uniqueCode!];
        const calculationHashName = map.get("calculationHashName") as string;
        const fileName = map.get("fileName") as string;
        const pMethods = globalDoneCallbackMapping.current.get(el.uniqueCode!);
