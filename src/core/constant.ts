...
export const UPLOAD_CONFIG_KEYS = {
  FILE_NAME: "fileName" as const,
  UNIQUE_CODE: "uniqueCode" as const,
  FILE_SIZE: FILE_SIZE_CONST,
  EXT_NAME: EXT_NAME_CONST,
  UPLOAD_FILE: UPLOAD_FILE_CONST,
  INNER_PROGRESS: INNER_PROGRESS_CONST,
  CURRENT_CONSUME_BYTES_KEY: CURRENT_CONSUME_BYTES,
  NETWORK_SPEED: NETWORK_SPEED_CONST,
  CALCULATION_HASH_NAME: "calculationHashName" as const,
};

// 网络测速专用前缀，避免与真实文件 hash 冲突
export const SPEED_TEST_PREFIX = "__speed_test__";

// 需要同步到全局 Map 的数值类型 key（用于同步到 globalFileSize/globalConsumeOffset）
export const SYNC_NUMERIC_VALUE_KEYS: ReadonlySet<string> = new Set([
  FILE_SIZE_CONST,
